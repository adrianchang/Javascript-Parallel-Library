...
function simd_float_array_div_benchmark(length, iteration) {
	let a = new Float32Array(length);
	let b = new Float32Array(length);
	let c = new Float32Array(length);
	...
	for (let i = 0; i < iteration; i++) {
		...
		simd_operations.simd_div_float_array(a_pointer, b_pointer, c_pointer, length);
		...
		if (check_array_div_correctness(a, b, c, length)) {
			// console.info('Benchmark simd_float_array_div_benchmark Execution time (hr): %ds', hrend[0], hrend[1] / 1000000000);
		} else {
			console.log("Benchmark simd_float_array_div_benchmark calculation error");
		}
	}

	console.info('Benchmark simd_float_array_div_benchmark Execution time (hr): %d ms', amount_time / iteration * 1000);
}
...
function baseline_float_array_div_benchmark(length, iteration) {
	...
	for (let i = 0; i < iteration; i++) {
		...
		// console.info('Baseline baseline_float_array_div_benchmark Execution time (hr): %ds', hrend[0], hrend[1] / 1000000000);
	}

	console.info('Baseline baseline_float_array_div_benchmark Execution time (hr): %d ms', amount_time / iteration * 1000);
}
...
function float_div_array_benchmark() {
	let length = 10000000;
	let iteration = 10;
	simd_float_array_div_benchmark(length, iteration);
